...
public class unitManager : MonoBehaviour
{
    ...
    public void CheckSchedule()
    {
        ...
        else if (!gameObject.activeSelf || Status.text == "defending" || Status.text == "attacking") return;
        // Small check to reset Schedule if AI is stuck.
        else if (AI.isStuck)
        {
            StopAllCoroutines();
            HideEmote();
            ...
            if (Schedule.Count > 0 && Schedule[0] != "Raider Roam") Schedule.RemoveAt(0);
        ...
        else if (Schedule.Count > 0 && !onSchedule)
        {
            ...
            if (Schedule[0].Contains("Create Unit"))
            {
                string whatUnit = Schedule[0].Replace("Create Unit ", "");
                StartCoroutine(CreateUnit(whatUnit));
            }
            else if (Schedule[0].Contains("Create"))
            {
                string whatBuilding = Schedule[0].Replace("Create ", "");
                StartCoroutine(CreateBuilding(whatBuilding));
            }
            else if (Schedule[0] == "Gather") { StartCoroutine(Gather()); }
            else if (Schedule[0] == "Research") { StartCoroutine(Research()); }
            else if (Schedule[0] == "Scout") { StartCoroutine(Scout()); }
            else if (Schedule[0].Contains("Guard"))
            {
            ...
            else if (Schedule[0] == "Raider Roam" && Status.text != "wandering" && Status.text != "attacking") { StartCoroutine(RaiderRoam()); }
            else if (Status.text == "wandering" && Kingdom.Units.Count > 0)
            {
    ...
    IEnumerator Roam()
    {
        ...
        while (!onSchedule)
        {
            ...
            if (Role == "Captain") { if (updatedRadius > 3) updatedRadius = 3; yield return new WaitForSeconds(1.5f); }
    ...
    IEnumerator CreateUnit(string Role)
    {
        ...
        yield return new WaitUntil(() => !Kingdom.Grid.CheckWall(12, 11) || Target.transform.position == new Vector3(12, Target.transform.position.y, 11));
    ...
    IEnumerator Research()
    {
        ...
        // Waits until the Research entrance is empty.
        yield return new WaitUntil(() => !Kingdom.Grid.CheckWall(Mathf.RoundToInt(Kingdom.ResearchBuilding.position.x), Mathf.RoundToInt(Kingdom.ResearchBuilding.position.z - 1))
                                         || Target.transform.position == new Vector3(Mathf.RoundToInt(Kingdom.ResearchBuilding.position.x), Target.transform.position.y, Mathf.RoundToInt(Kingdom.ResearchBuilding.position.z - 1)));

        Status.text = "researching";
        Target.position = new Vector3(Mathf.RoundToInt(Kingdom.ResearchBuilding.position.x), 0.5f, Mathf.RoundToInt(Kingdom.ResearchBuilding.position.z - 1));
        ...
        HideEmote();
        yield return new WaitForSeconds(2.5f);
        Kingdom.Researching(this.transform);
    }
