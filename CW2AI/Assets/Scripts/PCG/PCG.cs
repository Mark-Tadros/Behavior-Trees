...
public class PCG : MonoBehaviour
{
    ...
    void CreateGrid()
    {
        // Creates the initial layer for the world.
        ...
        // Creates the external objects such as Trees and Rocks. Excludes area at the edges and middle.
        for (int y = 1; y < Grid.worldSize - 1; y++)
        {
            for (int x = 1; x < Grid.worldSize - 1; x++)
            {
                if ((x > (Grid.worldSize / 2) - 3 && x < (Grid.worldSize / 2) + 3) && (y > (Grid.worldSize / 2) - 3 && y < (Grid.worldSize / 2) + 3))
                {
                    ...
                    // Creates Bushes (with a slight check to ensure it doesn't cross over water.
                    ...
                    // Creates Water in a rough co-joined pattern around the initial spawn to imitate realistic lakes.
